...
/**
 * A/B split for /agents.
 *
 * Change VARIANT_B_PERCENT to control the split:
 *   0    — split off. Everyone sees the control (/agents). No cookie. Default.
 ...
 * same variant. The browser URL always stays `/agents` for both variants.
 *
 * Facebook / Google / any ad platform only needs to point at
...
export function proxy(request: NextRequest) {
  ...
  // Force everyone to B (e.g. previewing B in prod without switching in code)
  if (VARIANT_B_PERCENT >= 100) {
    return NextResponse.rewrite(new URL("/agents/b", request.url));
  }
  ...
  // For variant B, rewrite to /agents/b — URL bar stays /agents
  const response =
    variant === "b"
...
// Only run this on /agents — cheap on every other route
export const config = {
  matcher: "/agents",
};
